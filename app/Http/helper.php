...
function postToDiscord($e)
{
	...
	if(preg_match("/Routing|Builder|guzzle/", $e->getFile())){
		return false;
	}
...
function noticeDiscord($content){
	$data = array("content" => date("Y/m/d H:i:s"). PHP_EOL .\Config::get('app.name').' '.\Config::get('app.url').  PHP_EOL ."```{$content}". PHP_EOL."Cron ver 1.1.1```", "username" => HOOKNAME);
	$curl = curl_init(WEBHOOK);
	curl_setopt($curl, CURLOPT_CUSTOMREQUEST, "POST");
	curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($data));
	curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
